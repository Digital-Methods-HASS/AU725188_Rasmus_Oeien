...
## Homicide-rates in western Europe 
```{r plot-homicides, eval = TRUE}
ggplot(data = Western_Europe, aes(x=Year,y=homicides_per_100k,color=Country))+
  geom_line()+
  theme_bw()
...
## Homicide-rates split by country/region
```{r eval = TRUE}
ggplot(data = Western_Europe, aes(x=Year,y=homicides_per_100k,color=Country))+
  geom_line()+
  facet_wrap(~Country)+
...
## The duration of the reigns of danish kings throughout history
```{r eval = TRUE}
Kings <- read_csv2("Kings_2.csv", na = "n/a")
names(Kings)

Kings %>% 
  mutate(duration = end_reign - crowning) %>% 
  mutate(midyear = end_reign - duration/2) %>% 
  ggplot(aes(x = midyear, y = duration )) +
  geom_smooth()+
  labs(title = "mean duration of the reigns of danish kings", x = "Year", y = "Duration of reign")+
  theme_bw()
  
```
I would say that there is a clear correlation between the homicide-rate and the duration of the reigns of Danish kings. It shows a Denmark that is gradually becoming more peacefull. That is the same trend that we see in the decline in homicides. That however is not nessecarily evidence of causation.
...
The conclusion of a society being civilised based on two sets of statistics, the murderrate and the durations of the reigns of its head of state, looks to me to be a shaky foundation to build a conclusion on. If we decide that society has to be peacefull to be civilised, we would still need further statistics on violent crime to come to a conclusion. Furthermore in my opinion a civilised society is also one that heals, provides for and educates its citizens. But if forced to come to a conclusion based on the rate of homicide, I would say that yes. We live in a more civilised society where fewer people feel the pressures that lead to commiting murder. 